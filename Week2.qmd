...
Please download this [Week 2 Download](downloadforstudent/week_2_studentversion.qmd) to get the .qmd for this weeks lesson. **IMPORTANT!! Practice importing this file into your project for this series! You need to make sure your datafiles and .qmds are all housed in your project you created last week.**
...
**Step 1:** install the package in your console (this is your script, which is fine for this purpose but more generally you want to install packages in your console because you only need to install once.)
...
```{r}
library(dplyr)
library("dplyr") #this works too but its not needed
```
...
-   Remember that loaded libraries do not disappear even after `rm(list = ls(all = TRUE))`. After you found some form of interference mentioned above, you may want to start over the analysis. To completely start over, you need to shutdown Rstudio because libraries are still loaded even after you clear environment. 
...
(from our glossary) **Indexing** — Selecting specific elements from a vector or dataset (e.g., `x[3]`).
...
**Exercise 3.** How can we get every second element? Look up what the function `seq()` does. Then generate a sequence with every second number going from 1 to the last element of the vector
```{r}
...
**Exercise 4.A** Generate a vector with 7 elements (You can mix strings and numbers!) 

```{r}
vec <- 10:16
```

**4.B.**Now substitute the 3rd element for the 6th

```{r}
vec[3] <- vec[6]
```

**4.C.** And finally add the number 99 at the end of this vector!
...
**1.** You learned that packages must be installed once and loaded every session. Use the help command to look up the function `filter()`. What package does it come from?

Install the package that contains `filter()`. Load the package into your session.
```{r}

```

**2.** Create a small data frame containing information about three fictional participants:

- Their names

- Their ages

- Whether they completed the task (TRUE/FALSE)

Print the data frame. Use `typeof()` on each column, and lastly use `View()` to inspect it. 
```{r}

```

**3.** Use the vector below:
```{r}
vec <- c(4, 9, 2, 7, 9, 1, 3, 9)
```
...
Extract elements 2 through 5
```{r}

```


Replace the 6th element with the number 100
```{r}

```


Use logical indexing to extract all values greater than 5
```{r}

```


Use `which()` to get the positions of all values equal to 9
```{r}
