...
The {tidyverse} is a *collection* of R packages that are designed to work together smoothly, so once you learn one, the others feel familiar. Packages within this collection include:

| Packages In The {tidyverse}: | Main Uses: |
|----|----|
| **dplyr** | Houses lots of functions to help with data manipulation. Helps you filter, select, group data variables, and aggregate data. This is a great package for data wrangling. |
| **ggplot2** | Create custom data visualizations, compatible with themes and other custom functions |
| **tidyr** | Data tidying and shaping.  This package helps you pivot, separate, join and organize data structures. |
| **readr** | Reads in datafiles (like CSV's) |
| **tibble** | This creates a more modern version of a dataframe that is cleaner and easier to use. |
| **forcats** | works with *cat*agorical variables (get it!). Lets you build more customizable plots |
| **stringr** | Makes it easy to detect, extract, replace, and clean up strings (text data). |
| **purrr** | Lets you repeat operations without writing loops. Great for applying a function to many things at once. |
| **lubridate** | Helps you parse, extract, and do math with dates and times. |
...
```{r}
ggplot(a, aes(x=state.region, y=IlliteracyRate, fill=IlliteracyRate)) + #sidenote: why the plus? shouldnt it be %>% ?!
...
ggplot(b, aes(x=state.division, y=IlliteracyRate, fill=AvgIncome))+
  geom_bar(position=position_dodge(), stat="identity")
```

```{r}
ggplot(df2, aes(x=Income, y=Illiteracy,color=state.region))+ 
